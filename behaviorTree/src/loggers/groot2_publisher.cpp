...
enum
{
  IDLE_FROM_SUCCESS = 10 + static_cast<int>(NodeStatus::E_SUCCESS),
  IDLE_FROM_FAILURE = 10 + static_cast<int>(NodeStatus::E_FAILURE),
  IDLE_FROM_RUNNING = 10 + static_cast<int>(NodeStatus::E_RUNNING)
};
  ...
  : StatusChangeLogger(tree.rootNode()), _p(new PImpl())
{
  ...
  for(const auto& subtree : tree.subtrees)
  {
    ...
    for(const auto& node : subtree->nodes)
    {
      ...
      ptr_offset += Monitor::Serialize(buffer_ptr, ptr_offset, uint8_t(NodeStatus::E_IDLE));
    }
  }
  //-------------------------------
