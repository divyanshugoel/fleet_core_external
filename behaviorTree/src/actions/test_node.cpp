  ...
  : StatefulActionNode(name, config), _test_config(std::move(test_config))
{
  ...
  if(_test_config.return_status == NodeStatus::E_IDLE)
  {
    throw RuntimeError("TestNode can not return E_IDLE");
  }
...
BT::NodeStatus BT::TestNode::onCompleted()
{
  ...
  if(status == NodeStatus::E_SUCCESS && _success_executor)
  {
    _success_executor(env);
  }
  else if(status == NodeStatus::E_FAILURE && _failure_executor)
  {
